...
def __train1():
    ...
    train_hypext_logits_file = os.path.join(config.DATA_DIR,
                                            'bbn/bert-tmhypextbbn-bbn-train-all.txt')
    ...
    gres = expdata.ResData(config.BBN_FILES['type-vocab'], None)
    type_infer_train = TypeInfer(gres.type_vocab, gres.type_id_dict, single_type_path=True)
    ...
    gres_test = expdata.ResData(config.BBN_FILES['type-vocab'], None)
    ...
    type_infer_test = TypeInfer(gres_test.type_vocab, gres_test.type_id_dict, single_type_path=True)
...
def __train():
    ...
    # train_srl_preds_file = os.path.join(config.DATA_DIR, 'figer/figer-train-pretrained-srl-preds.txt')
    ...
    # test_srl_preds_file = os.path.join(config.DATA_DIR, 'figer/figer-test-pretrained-srl-preds.txt')
